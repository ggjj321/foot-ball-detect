...
# save whole coordinate to avoid horn hidden
goal_upper_left_x  = []
goal_upper_left_y  = []
goal_upper_right_x = []
goal_upper_right_y = []
...
async def find_coordinate(target_squares: str, detect_video: UploadFile = File(...)):
    target_squares_list = target_squares.split(",")
    with open('detect.mp4', 'wb+') as file_obj:
        file_obj.write(detect_video.file.read())
    target_squares_list = list(map(int, target_squares_list))
    cap = cv2.VideoCapture('detect.mp4')
    score, coordinate_x, coordinate_y = detect_video_to_calculate_data(target_squares_list, cap)
...
def detect_video_to_calculate_data(targets, cap):
    shot = False
    score = 100
    while cap.isOpened():
        ...
        boal_area, goal_area, horn_points = detectObject(df, image)        

        if goal_area:
            rate = rating(boal_area, goal_area, image)
            upper_left, upper_right, down_left, down_right = getAllHornsCoordinates(horn_points, goal_area, image)

            drawBoarderLines(image, upper_left, upper_right, down_left, down_right)
            ...
            splitGoalArea(image, hor_up, hor_down, straight_left, straight_right)

            # moment that the ball shoots in the goal
            if rate and rate > 10 and not shot: # rate should be 10
                boal_center = getCenter(boal_area)
                                
                xaxis = getLength(upper_left[0], upper_left[1], upper_right[0], upper_right[1])
                yaxis = getLength(upper_left[0], upper_left[1], down_left[0], down_left[1])
                boalToBarX = getLength(boal_center[0], boal_center[1], upper_left[0], boal_center[1])
                boalToBarY = getLength(boal_center[0], boal_center[1], boal_center[0], upper_left[1])
                coordinate_x = boalToBarX / xaxis
                coordinate_y = boalToBarY / yaxis

                if 0 <= coordinate_x <= 1 and  0 <= coordinate_y <= 1:
                    shot = True
                else:
                    score = 0
                ...
                drawBoarderLines(result_img, upper_left, upper_right, down_left, down_right)

                splitGoalArea(result_img, hor_up, hor_down, straight_left, straight_right)
    ...
    if score != 0:
        square = judge_boal_shot_at_which_square(coordinate_x, coordinate_y)
        score = calculate_score(square, targets)

    return score, coordinate_x, coordinate_y